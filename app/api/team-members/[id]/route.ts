...
export async function PUT(request: Request, { params }: { params: { id: string } }) {
  ...
  const { first_name, last_name, name, email, is_me, role } = await request.json()
  ...
  db.prepare(
    'UPDATE team_members SET name = ?, first_name = ?, last_name = ?, email = ?, is_me = ?, role = ? WHERE id = ?'
  ).run(fullName, resolvedFirst, resolvedLast, email?.trim() || null, is_me ? 1 : 0, role || 'user', id)
  ...
  const member = db.prepare('SELECT id, name, first_name, last_name, email, is_me, role FROM team_members WHERE id = ?').get(id)
